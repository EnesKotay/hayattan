...
export default async function AdminYazarlarPage({
  ...
  const eskiYazarlar = tumu.filter((y) => (y as { ayrilmis?: boolean }).ayrilmis);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Eski Yazarlar */}
      {eskiYazarlar.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 font-serif text-lg font-bold text-amber-700">
            Eski Yazarlar ({eskiYazarlar.length})
          </h2>
          <p className="mb-4 text-sm text-muted">
            Şirketten ayrılmış yazarlar. Yazıları <Link href="/eski-yazilar" className="text-primary hover:underline">/eski-yazilar</Link> sayfasında listelenir.
          </p>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-amber-200 bg-amber-100">
                    <th className="px-4 py-3 font-semibold text-amber-900">Ad Soyad</th>
                    <th className="px-4 py-3 font-semibold text-amber-900">E-posta</th>
                    <th className="px-4 py-3 font-semibold text-amber-900">Yazı sayısı</th>
                    <th className="px-4 py-3 font-semibold text-amber-900">İşlem</th>
                  </tr>
                </thead>
                <tbody>
                  {eskiYazarlar.map((yazar) => (
                    <tr key={yazar.id} className="border-b border-amber-100 last:border-0">
                      <td className="px-4 py-3 font-medium text-foreground">{yazar.name}</td>
                      <td className="px-4 py-3 text-muted">{yazar.email ?? "—"}</td>
                      <td className="px-4 py-3 text-muted">{yazar._count.yazilar} yazı</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span className="text-[#ccc]">|</span>
                          <form action={deleteYazar.bind(null, yazar.id)} className="inline">
                            <DeleteConfirmButton
                              confirmMessage={`"${yazar.name}" yazarını silmek istediğinize emin misiniz? Bu yazara ait ${yazar._count.yazilar} yazı da silinecektir. Bu işlem geri alınamaz.`}
                              className="font-medium text-red-600 hover:underline"
                            >
